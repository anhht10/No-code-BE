...
} from './schemas/refresh-token.schema'; // Nhớ dùng Document
...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    const isExist = await this.userService.findInternalByEmail(
      registerDto.email,
    );
    ...
    const newUser = await this.userService.create({
      ...
      username: registerDto.name
        ? registerDto.name.replace(/\s+/g, '').toLowerCase()
        : `user${Date.now()}`,
  ...
  async login(
    ...
    if (!isMatch) {
      console.log(
        `Login failed for email: ${loginDto.email} - Incorrect password`,
      );
      throw new UnauthorizedException('Sai email hoặc mật khẩu}');
    }
  ...
  async logout(refreshToken: string) {
    ...
    if (result.deletedCount === 0) {
      throw new BadRequestException(
        'Token không tồn tại hoặc đã được đăng xuất trước đó',
      );
    }
    return {};
  }
  ...
  private async generateAuthTokens(
    ...
    expirationDate.setTime(
      expirationDate.getTime() + ms(refreshExpires as StringValue),
    );
