...
export class PaymentService {
  ...
  async createPayosPayment(payload: CreatePayosPaymentDto) {
    ...
    const returnUrl =
      process.env.PAYOS_RETURN_URL || 'https://chidung7271.id.vn/success';
    const cancelUrl =
      process.env.PAYOS_CANCEL_URL || 'https://chidung7271.id.vn/cancel';

    if (!clientId || !apiKey || !checksumKey) {
      throw new InternalServerErrorException(
        'PayOS environment variables are not configured',
      );
    }
    ...
    const description = this.buildDescription(
      payload.description || `Thanh toan ${payload.courseTitle}`,
    );
    ...
    if (!response.ok || result.code !== '00' || !result.data) {
      throw new BadRequestException(
        result.desc || 'Failed to create PayOS payment request',
      );
    }
  ...
  async handlePayosWebhook(payload: PayosWebhookDto) {
    ...
    payment.paymentLinkId =
      payload.data?.paymentLinkId || payment.paymentLinkId;
  ...
  private buildDescription(description: string) {
    return description.replace(/\s+/g, ' ').trim().slice(0, 25);
  }
